...
**Se quella persona l'hai già creata a mano da *Utenti*, con un'altra email, il
comando non ne crea una seconda**: si ferma e ti dice quale indirizzo ha
trovato. Due profili per la stessa persona vogliono dire appuntamenti segnati
su quello che lei non apre mai — è successo il 18 settembre 2026, ed è il
motivo per cui adesso controlla anche il nome.
...
**Quando due comuni sono lo stesso comune** (corretto il 12 settembre 2026). Il
programma tollera le scritture diverse — «LECCE (LE)» e «Lecce» sono lo stesso
posto, e «Monteroni» vale «Monteroni di Lecce» — ma **non confonde più due
comuni diversi**. Prima chi cercava a Lecce si vedeva proposti anche gli
immobili di Monteroni di Lecce, San Cesario di Lecce, Caprarica, Castrì,
Minervino, San Donato e Muro Leccese: sono tutti nomi che contengono la parola
«Lecce», e al confronto bastava quello. Adesso no.

Un immobile a cui **manca il comune in scheda** continua a essere proposto:
non è in un comune diverso, è in un comune che non sappiamo, e accanto compare
l'avvertenza *«Comune non indicato sull'immobile»*. Si toglie di mezzo
completando l'indirizzo — il cruscotto dice quali sono.
...
## 9-sexies · Quando il programma dice di no

Fino al 3 settembre 2026, un salvataggio rifiutato dal server dava una
**pagina di errore senza spiegazioni**. Il motivo del rifiuto c'era — «Serve
l'indirizzo dell'immobile» — ma non usciva dal server: a programma pubblicato
React nasconde il testo degli errori, per non lasciarsi sfuggire dettagli
interni, e a schermo restava una pagina bianca in inglese. Chi ci finiva
dentro non sapeva cosa correggere, e tornando indietro trovava il modulo
vuoto.

Adesso i rifiuti si comportano da rifiuti:

- **Il motivo compare sopra il pulsante Salva**, in rosso, scritto per esteso.
  Sopra il pulsante e non in cima alla pagina: su un modulo lungo, in cima
  sarebbe fuori schermo, e il salvataggio sembrerebbe non aver fatto niente.
- **Quello che si era scritto resta nei campi.** Si corregge la riga
  segnalata e si salva di nuovo. Sulla scheda di un immobile sono venticinque
  campi: perderli per un titolo dimenticato era peggio del rifiuto.
- **In archivio non viene scritto niente.** Il controllo viene prima
  dell'inserimento, non a metà.

Le cose che possono venire rifiutate sono quattro: un cliente senza né cognome
né ragione sociale, un immobile senza titolo, un immobile senza via, un
collegamento al video che non è un indirizzo.

### E se è un guasto vero

Per tutto il resto — un guasto imprevisto, o il tentativo di toccare la scheda
di un collega — c'è ora una **schermata di cortesia**, «Qualcosa non ha
funzionato»: in italiano, con la navigazione al suo posto, il pulsante
*Riprova* e il ritorno al cruscotto. Prima era la pagina bianca di Next.

In fondo alla schermata c'è un **codice del guasto**. Serve: è lo stesso che
il programma scrive nel proprio registro, quindi permette di ritrovare
l'errore vero fra tutti quelli della giornata (capitolo 12).

Allo stesso modo, un indirizzo che non porta da nessuna parte e una scheda
che non c'è più non mostrano più il *404 This page could not be found* di
serie, ma due schermate italiane con la via d'uscita.

> **Nota sulle schede dei colleghi.** Una scheda che esiste ma è di un altro
> mostra la stessa schermata di una che non esiste. È voluto: distinguere i
> due casi sarebbe un modo per contare l'archivio altrui una prova alla volta.
...
4. Il ruolo **titolare** non dà più accesso ai dati altrui: serve solo a creare
   e disattivare gli utenti, e a prendere i link dei calendari (qui sotto). Il
   **registro accessi** ce l'hanno tutti, ma ciascuno vede soltanto le proprie
   mosse.

### L'unica apertura nel muro: i calendari

Aggiunta il 12 settembre 2026, chiesta da Camillo e voluta con cognizione di
causa. In fondo alla pagina **Utenti** — che è del solo titolare — c'è
l'indirizzo del calendario di ogni persona, con il pulsante per copiarlo. Serve
a mettere i calendari dei collaboratori nel proprio Google Calendar, uno per
colore.
...
- **il link non esiste finché non lo si crea.** La pagina non genera niente da
  sola: ogni riga parte da un pulsante *Crea il calendario*. Aprire la pagina
  Utenti non deve aprire una porta per ognuno;
- **ogni creazione finisce nel registro accessi**, col nome di chi l'ha chiesta
  e di chi riguarda;
- **si può spegnere**: *genera un indirizzo nuovo* rende muto il vecchio.
  Attenzione che ferma anche i calendari già collegati, compresi quelli sul tuo
  Google, finché non li ricolleghi.

---

## 10-ter · Collegare Google Calendar

Aggiunto il 15 settembre 2026, dopo che l'abbonamento si è rivelato troppo
lento: **Google ricontrolla i calendari a cui ci si abbona quando decide lui**,
anche dopo ore, e per vedere un appuntamento nuovo bisognava togliere il
calendario da Google e rimetterlo. Il file gli chiede di ricontrollare ogni
quarto d'ora e Google ignora la richiesta — non è una cosa che si possa
sistemare da questa parte.

La seconda strada rovescia il verso: invece di aspettare che Google venga a
guardare, **il gestionale scrive l'appuntamento dentro Google** appena lo salvi.
...
- Il calendario di una persona **nasce al suo primo appuntamento**, non quando
  colleghi: chi non ne ha non si ritrova un calendario vuoto fra i suoi.

### Cosa il gestionale NON può fare, ed è la parte importante

Il permesso che chiede si chiama `calendar.app.created`, e vuol dire
**«posso creare calendari miei e gestire solo quelli»**. Il tuo calendario
personale — quello con gli appuntamenti di famiglia, le riunioni, tutto — il
gestionale **non lo può leggere e non lo può toccare**. È scritto sulla
...
usando prima di cominciare.

**1 · Crea il progetto.** Vai su `console.cloud.google.com`. In alto, accanto al
logo, c'è il selettore dei progetti: aprilo e scegli *Nuovo progetto*. Chiamalo
come vuoi — per esempio `Gestionale Mondo` — e premi *Crea*. Aspetta che in alto
compaia il nome del progetto nuovo: da lì in poi stai lavorando dentro quello.

**2 · Accendi il calendario.** Menu a sinistra → *API e servizi* → *Libreria*.
Cerca **Google Calendar API**, aprila e premi **Abilita**. Senza questo passo
tutto il resto funziona e poi, al primo appuntamento, Google risponde che l'API
non è stata usata in questo progetto.

**3 · La schermata di consenso.** *API e servizi* → *Schermata consenso OAuth*.
Scegli **Esterno** e premi *Crea*. Ti chiede tre cose: il nome
dell'applicazione (quello che leggerai sulla schermata di consenso — scrivi
`Gestionale Mondo Immobiliare`), la tua email di assistenza, e in fondo di nuovo
...
> passato la sua revisione — che serve a chi distribuisce un programma a
...
### Preparare i calendari prima che servano

Il calendario di una persona **nasce da sé al suo primo appuntamento**: chi non
ne ha non si ritrova un calendario vuoto fra i suoi. Se però li vuoi tutti
subito — per metterli in Google, dargli un colore, decidere quali tenere accesi
— nella pagina c'è il pulsante **«Crea adesso i calendari mancanti»**, che li
crea in un colpo per tutte le persone attive.

Non fa doppioni: chi ce l'ha già viene saltato, e l'appuntamento che arriva dopo
finisce nel calendario che c'è, non in uno nuovo.

### Con quale account sei collegato

**È la prima cosa scritta nella pagina**, nel riquadro azzurro: *«Collegato con
l'account …»*. Vale la pena guardarla prima di ogni altra cosa, perché il
problema più facile da avere è anche il meno riconoscibile: **autorizzare con un
account e cercare i calendari in un altro**. È successo il 16 settembre 2026, e
ci sono volute due ore per capirlo — i calendari c'erano, in un altro Gmail.

Se l'account è quello sbagliato: *scollega Google*, poi *Collega Google*, e sulla
schermata di Google **scegli quello giusto** (in alto ti mostra con quale stai
per autorizzare). I calendari vengono ricreati nel nuovo account, e per
rimandarci gli appuntamenti si usa il solito pulsante.

I calendari rimasti nell'account vecchio non spariscono da soli: si cancellano a
mano da lì, quando vuoi.

### Se qualcosa non va

La pagina non dà codici d'errore: dice **quale riga correggere**. I tre casi che
capitano davvero:

| Cosa vedi | Cosa vuol dire |
|---|---|
| *Google non riconosce il Client ID o il segreto* | Copia-incolla incompleto, o spazi in coda. Si riprendono da *Credenziali*. |
| *L'indirizzo di ritorno non è fra quelli autorizzati* | Il passo 5: l'URI va scritto identico, ed è quello che la pagina ti mostra da copiare. |
| *Il permesso non vale più* | O è stato tolto da `myaccount.google.com/permissions`, oppure l'app è rimasta in stato *Test* (passo 4). Ricollega, e se ricapita ogni settimana, pubblica l'app. |

### Le due strade convivono

L'abbonamento `.ics` resta dov'era e continua a funzionare: chi lo ha già
collegato non deve cambiare niente. Le due cose non si danno fastidio, ma
**collegandole tutte e due lo stesso appuntamento comparirebbe due volte** —
una dal calendario in abbonamento, una da quello scritto dal gestionale. Scegline
una: se colleghi Google davvero, togli da Google Calendar i calendari in
abbonamento.
...
| A schermo compare **«Qualcosa non ha funzionato»** | Non è stato salvato niente. Prendi il **codice del guasto** in fondo a quella schermata e cercalo nel registro: `ssh root@77.81.234.151 "journalctl -u mondo-crm \| grep <codice>"`. Lì c'è l'errore per esteso, che a schermo non esce apposta |
| Un salvataggio viene rifiutato con un riquadro rosso | Non è un guasto: è un controllo. Il riquadro sta sopra il pulsante *Salva* e dice cosa manca; quello che avevi scritto è ancora nei campi (capitolo 9-sexies) |
