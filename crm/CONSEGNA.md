...
1. **`serverExternalPackages: ["better-sqlite3"]`** in `next.config.ts`. Senza,
   la compilazione fallisce: è un modulo nativo e non va impacchettato.
...
dell'archivio, e lì dentro c'è tutto; chi amministra gli utenti può anche
cambiare la password di un altro. Se serve una separazione che regga fra due
